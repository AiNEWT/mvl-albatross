hsemfit<-function(RespDist="gaussian",BinomialDen=NULL, DataMain, MeanModel,DispersionModel,
  ...
  if (is.null(MeanModel[[6]][1])==TRUE) {
    if (is.null(DispersionModel[[12]][1])==FALSE) {
        if(DispersionModel[[12]][1]=="AR") {
             ...
             res<-hsemfit_run_sv(RespDist=RespDist,BinomialDen=BinomialDen, DataMain=DataMain, MeanModel=MeanModel,
    ...
    if (DispersionModel[[12]][1]=="GARCH") {
             ...
             res<-hsemfit_run_GARCH(RespDist=RespDist,BinomialDen=BinomialDen, DataMain=DataMain, MeanModel=MeanModel,
    ...
    if (DispersionModel[[12]][1]=="IND" && MeanModel[[13]][1]=="IND" && is.null(MeanModel[[16]][1])==TRUE) {
             res<-hsemfit_run(RespDist=RespDist,BinomialDen=BinomialDen, DataMain=DataMain, MeanModel=MeanModel,
             DispersionModel=DispersionModel,BetaFix=BetaFix,PhiFix=PhiFix,LamFix=LamFix,mord=mord,dord=dord,REML=REML,
             Maxiter=Maxiter,convergence=convergence,Iter_mean=5,corr="IND",EstCorr=EstCorr,Dmethod=Dmethod)
    }
    }
    if (MeanModel[[13]][1]=="Matern" && is.null(MeanModel[[14]][1])==FALSE && is.null(MeanModel[[15]][1])==FALSE) res<-hsemfit_Matern(RespDist=RespDist,BinomialDen=BinomialDen, DataMain=DataMain, MeanModel=MeanModel,
             DispersionModel=DispersionModel,BetaFix=BetaFix,PhiFix=PhiFix,LamFix=LamFix,mord=mord,dord=dord,REML=REML,
    ...
    if (is.null(MeanModel[[16]][1])==FALSE && is.null(DispersionModel[[16]][1])==TRUE) res<-hsemfit_spline(RespDist=RespDist,BinomialDen=BinomialDen, DataMain=DataMain, MeanModel=MeanModel,
             DispersionModel=DispersionModel,BetaFix=BetaFix,PhiFix=PhiFix,LamFix=LamFix,mord=mord,dord=dord,REML=REML,
             Maxiter=Maxiter,convergence=convergence,Iter_mean=5,corr="IND",EstCorr=EstCorr,Dmethod=Dmethod)
    if (is.null(MeanModel[[16]][1])==FALSE && is.null(DispersionModel[[16]][1])==FALSE) res<-hsemfit_spline2(RespDist=RespDist,BinomialDen=BinomialDen, DataMain=DataMain, MeanModel=MeanModel,
             DispersionModel=DispersionModel,BetaFix=BetaFix,PhiFix=PhiFix,LamFix=LamFix,mord=mord,dord=dord,REML=REML,
             Maxiter=Maxiter,convergence=convergence,Iter_mean=5,corr="IND",EstCorr=EstCorr,Dmethod=Dmethod)
  } else {
             res<-hsemfit_run_Lmatrix(RespDist=RespDist,BinomialDen=BinomialDen, DataMain=DataMain, MeanModel=MeanModel,
             DispersionModel=DispersionModel,BetaFix=BetaFix,PhiFix=PhiFix,LamFix=LamFix,mord=mord,dord=dord,REML=REML,
...
hsemmodeling<-function(Model="mean",Link=NULL,LinPred="constant",RandDist=NULL,
...
plothsem<-function (OUTPUT,type="mean",random=NULL) {
    random=NULL
    par(mfrow=c(2,2))
    q<-OUTPUT$q
    if (type=="mean") {
...
hsemfit_run_sv<-function(RespDist="gaussian",BinomialDen=NULL, DataMain, MeanModel,DispersionModel,
...
hsemfit_spline<-function(RespDist="gaussian",BinomialDen=NULL, DataMain, MeanModel,DispersionModel,
...
#          res1<-crPlotsHGLM(m)
 ...
 #         res1<-crPlotsHGLM(m)
  ...
  #        res1<-crPlotsHGLM(m)
    ...
    model_mu<-hsemmodeling(Model="mean",Link="identity",LinPred=ystar~x)
    model_phi<-hsemmodeling(Model="dispersion",Link="log")
    if (RespDist=="gaussian") {
...
#         print(beta_coeff,4)
         ...
         res1<-hsemfit(RespDist="gaussian",MeanModel = model_mu,DispersionModel = model_phi,DataMain=f)
    }
    else {
...
hsemfit_spline1<-function(RespDist="gaussian",BinomialDen=NULL, DataMain, MeanModel,DispersionModel,
BetaFix=NULL,PhiFix=NULL,LamFix=NULL,mord=1,dord=1,REML=TRUE,Maxiter=200,convergence=1e-06,Iter_mean=5,corr=NULL,EstCorr=EstCorr,Dmethod=Dmethod) {
    mc <- match.call()
    formulaMean<-MeanModel[3][[1]]
    fr <- HGLMFrames(mc, formulaMean,contrasts=NULL)
...
hsemfit_spline2<-function(RespDist="gaussian",BinomialDen=NULL, DataMain, MeanModel,DispersionModel,
BetaFix=NULL,PhiFix=NULL,LamFix=NULL,mord=1,dord=1,REML=TRUE,Maxiter=200,convergence=1e-06,Iter_mean=5,corr=NULL,EstCorr=EstCorr,Dmethod=Dmethod) {
    mc <- match.call()
    formulaMean<-MeanModel[3][[1]]
    fr <- HGLMFrames(mc, formulaMean,contrasts=NULL)
...
hsemfit_run_GARCH<-function(RespDist="gaussian",BinomialDen=NULL, DataMain, MeanModel,DispersionModel,
...
hsemfit_Matern<-function(RespDist="gaussian",BinomialDen=NULL, DataMain, MeanModel,DispersionModel,
...
hsemfit_run_Lmatrix<-function(RespDist="gaussian",BinomialDen=NULL, DataMain, MeanModel,DispersionModel,
...
hsemfit_run <-
function(RespDist="gaussian",BinomialDen=NULL, DataMain, MeanModel,DispersionModel,
BetaFix=NULL,PhiFix=NULL,LamFix=NULL,mord=1,dord=1,REML=TRUE,Maxiter=200,convergence=1e-06,Iter_mean=5,Dmethod="deviance",corr="IND",EstCorr=EstCorr) {
    mc <- match.call()
    formulaMean<-MeanModel[3][[1]]
    fr <- HGLMFrames(mc, formulaMean,contrasts=NULL)
    namesX <- names(fr$fixef)
