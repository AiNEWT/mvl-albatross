# Here is the part of algorithm keeping the correlation fixed #
# W tym pliku zmienilismy pochodna vhat wzgledem lambda na joint pochodna, blad zostal naprawiony #

# This is a code to source #

# Also here we need to change the convergence criterion - into DRgamma correlated not on the transformed scale plus Vs - this needs to be ammended #

# Fix second derivative of the vhat with respect to rho  #

# Added a 

# NOTE: The convergence of dispersion parameters should be on the original scale #
IWLS_CorrZIP<-function(Loadings,Correlations,corrModel,YList=NULL,
...
## hlikelihood<-hlikelihood+sum(dpois(YList[[i]],mu[(cModelsDims[i]+1):cModelsDims[i+1]],log=TRUE)/Phi[(cModelsDims[i]+1):cModelsDims[i+1]],log=TRUE)
                ...
                hlikelihood3<-hlikelihood-0.5*determinant((DD1/(2*pi)),log=TRUE)$modulus
                hlikelihood4<-hlikelihood-0.5*determinant((DD/(2*pi)),log=TRUE)$modulus
...
if (StandardErrors==FALSE){
    ...
    tBeta=Beta/StdErrBeta
    pBeta<-pnorm(-abs( tBeta ))*2
    OUT<-list(Beta=Beta,V=VT,DRgamma=DRgamma,DYgamma=DYgamma,Correlations=unlist(Correlations),StdErrCorr=StdErrCorr,StdErrCorrZF=StdErrCorrZF,
            StdErrVs=StdErrVs,StdErrBeta=StdErrBeta, tBeta=tBeta, pBeta=pBeta,StdErrODEst=StdErrODEst,StdErrDRgamma=StdErrDRgamma,
    ...
    # 3 sets of standard errors - Random effects, fixed effects and variance components #
